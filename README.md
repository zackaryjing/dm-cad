...
- `loss.param_scale`: 参数损失归一化尺度。当前参数值量级较大时，应设置到与数据尺度接近的值，避免归一化后的参数误差尺度失真
- `loss.param_huber_delta`: 归一化参数空间中的 Huber 阈值。若 `param_scale=128` 且 `param_huber_delta=0.25`，则二次区间大约对应原始误差 `32`
