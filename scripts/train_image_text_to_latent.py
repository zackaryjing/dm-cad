...
from deepcad_latent.data import ImageTextLatentDataset, RunningAverage, collate_image_text_latent
from deepcad_latent.model import MultiModalLatentRegressor


def parse_args():
    parser = argparse.ArgumentParser(description="Train image+text to DeepCAD latent regressor")
    ...
    parser.add_argument("--train-text-root", type=Path, required=True)
    parser.add_argument("--test-text-root", type=Path, required=True)
    parser.add_argument("--init-image-checkpoint", type=Path, required=True)
    parser.add_argument("--freeze-image-epochs", type=int, default=2)
    parser.add_argument("--text-dropout", type=float, default=0.3)
...
def set_image_branch_frozen(model, freeze: bool):
    target = model.module if isinstance(model, DDP) else model
    target.freeze_image_branch(freeze)
...
def main():
    ...
    train_dataset = ImageTextLatentDataset(
        ids_file=args.train_ids,
        latent_root=args.train_latent_root,
        text_root=args.train_text_root,
        data_root=args.data_root,
        lmdb_path=args.lmdb_path,
        img_size=args.img_size,
        n_views=args.n_views,
    )
    test_dataset = ImageTextLatentDataset(
        ids_file=args.test_ids,
        latent_root=args.test_latent_root,
        text_root=args.test_text_root,
        data_root=args.data_root,
        lmdb_path=args.lmdb_path,
        img_size=args.img_size,
        n_views=args.n_views,
    ...
    model = MultiModalLatentRegressor(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch == args.freeze_image_epochs + 1:
            set_image_branch_frozen(model, freeze=False)
            optimizer = build_optimizer(model, args.lr, args.weight_decay)
        ...
        epoch_metrics = {
            "epoch": epoch,
            "train_mse": train_mse_meter.avg,
            "train_cosine": train_cos_meter.avg,
